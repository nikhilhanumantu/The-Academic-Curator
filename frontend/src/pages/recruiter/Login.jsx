...
import { FloatingInput, RoleToggle, AuthAlert } from '../../components/auth/AuthComponents';
  ...
  return (
    <AuthLayout role="recruiter">
      <div className="auth-card-header">
        <RoleToggle role="recruiter" onStudentPath="/student/login" onRecruiterPath="/recruiter/login" />
        <h1 className="auth-card-title">
          Recruiter <span className="ac-highlight-recruiter">Portal</span>
        </h1>
        <p className="auth-card-subtitle">Sign in to discover exceptional talent.</p>
      </div>
      ...
      <form className="auth-form" onSubmit={handleSubmit} noValidate>
        ...
        <label className="auth-check-row">
          <input type="checkbox" id="rl-remember" checked={remember} onChange={(e) => setRemember(e.target.checked)} />
          <span>Keep me signed in</span>
        </label>

        {/* Recruiter button — indigo/blue gradient */}
        <button
          type="submit"
          disabled={loading}
          className="auth-btn auth-btn--recruiter"
        >
          {loading ? <span className="auth-btn-spinner" /> : 'Sign In as Recruiter'}
        </button>
      </form>

      <p className="auth-link" style={{ marginTop: 24 }}>
        Don't have an account?{' '}
        <Link to="/recruiter/register" style={{ color: '#818cf8' }}>Create account</Link>
      </p>
    </AuthLayout>
  );
}
